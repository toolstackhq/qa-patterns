...
const reportFlagIndex = process.argv.indexOf("--report-file");
const reportFile =
  reportFlagIndex >= 0 && process.argv[reportFlagIndex + 1]
    ? path.resolve(repoRoot, process.argv[reportFlagIndex + 1])
    : path.resolve(repoRoot, ".github/dependency-monitor-report.json");
...
const report = {
  ...
  playwright: {
    ...
    updateAvailable: currentPlaywrightVersion !== latestPlaywrightVersion
  },
  vulnerabilities
};
...
writeOutput("playwright_update_available", String(report.playwright.updateAvailable));
